...
# Adapted from https://github.com/caogang/wgan-gp/blob/master/gan_toy.py
def calc_gradient_penalty_with_gt(net_dis, real_data, fake_data, batch_size, lmbda, use_cuda=False, gt_code=None):
    # Compute interpolation factors
    alpha = torch.rand(batch_size, 1, 1)
    alpha = alpha.expand(real_data.size())
    alpha = alpha.cuda() if use_cuda else alpha
    ...
    # Evaluate discriminator
    disc_interpolates = net_dis(interpolates, gt_code)
...
# Adapted from https://github.com/caogang/wgan-gp/blob/master/gan_toy.py
def calc_gradient_penalty_with_gt_v2(net_dis, real_data, fake_data, batch_size, lmbda, use_cuda=False, real_gt=None, fake_gt=None):
    # Compute interpolation factors
    alpha = torch.rand(batch_size, 1, 1)
    alpha = alpha.expand(real_data.size())
    alpha = alpha.cuda() if use_cuda else alpha
    ...
    # Evaluate discriminator
    disc_interpolates = net_dis(interpolates, real_gt_code)
...
# added by Vajira
def get_plots(true_ecg, fake_ecg):
    #print(true_ecg.shape)
    #for i in range(num_of_samples):
    fig, axs = plt.subplots( 2)
    #sns_plot = sns.lineplot(x = [i for i in range(len(true_ecg))], y=true_ecg, ax=axs[0])
    ...
    true_ecg_8_chs = true_ecg.reshape(8, 512)
    fake_ecg_8_chs = fake_ecg.reshape(8, 512)
...
# vajira
def get_plots_RHTM(true_ecg, fake_ecg):
    #print(true_ecg.shape)
    #for i in range(num_of_samples):
    fig, axs = plt.subplots( 2)
    #sns_plot = sns.lineplot(x = [i for i in range(len(true_ecg))], y=true_ecg, ax=axs[0])
    ...
    #print("true ecg shape = ", true_ecg.shape)
    #print("fake ecg shape =", fake_ecg.shape)
    true_ecg_8_chs = true_ecg.reshape(8, 4096)
    fake_ecg_8_chs = fake_ecg.reshape(8, 4096)
...
def get_plots_RHTM_10s(true_ecg, fake_ecg):
    ...
    #print("true ecg shape = ", true_ecg.shape)
    #print("fake ecg shape =", fake_ecg.shape)
    true_ecg_8_chs = true_ecg.reshape(2, 5000)
    fake_ecg_8_chs = fake_ecg.reshape(2, 5000)
...
def get_plots_all(true_ecgs, fake_ecgs, num_of_plots = 4):
    #print(true_ecg.shape)
    #for i in range(num_of_samples):
    ...
    for b in range(bs):
        ...
        true_ecg_8_chs = true_ecgs[b].reshape(8, 512)
        fake_ecg_8_chs = fake_ecgs[b].reshape(8, 512)
...
def get_plots_all_RHTM(true_ecgs, fake_ecgs, num_of_plots = 4):
    #print(true_ecg.shape)
    #for i in range(num_of_samples):
    bs = true_ecgs.shape[0]
    if bs > num_of_plots:
    ...
    for b in range(bs):
        ...
        true_ecg_8_chs = true_ecgs[b].reshape(8, 4096)
        fake_ecg_8_chs = fake_ecgs[b].reshape(8, 4096)
...
def get_plots_all_RHTM_10s(true_ecgs, fake_ecgs, num_of_plots = 4):
    ...
    for b in range(bs):
        ...
        true_ecg_8_chs = true_ecgs[b].reshape(2, 5000)
        fake_ecg_8_chs = fake_ecgs[b].reshape(2, 5000)
