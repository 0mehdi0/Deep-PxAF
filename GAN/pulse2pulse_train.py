...
#==============================
parser.add_argument("--data_dirs", default=["output", 
...
#==========================================
def prepare_data():
    dataset =  Data_set()#ecg_data(opt.data_dirs, norm_num=6000, cropping=None, transform=None)#Data_set()
    #print(dataset)
...
#===============================================
def prepare_model():
    netG = Pulse2PuseGenerator(model_size=opt.model_size, ngpus=opt.ngpus,num_channels=2,upsample=True)
    netD = Pulse2PulseDiscriminator(model_size=opt.model_size, num_channels=2,ngpus=opt.ngpus)
...
def train(netG, netD, optimizerG, optimizerD, dataloader):
    logging.basicConfig(filename="code_logger_1.txt",
                        filemode='a',
    ...
    for epoch in tqdm(range(opt.start_epoch + 1, opt.start_epoch + opt.num_epochs + 1)):
        ...
        for i, sample in tqdm(enumerate(dataloader, 0)):
            #print((sample['ecg_signals'].shape),'lendataloader:',len(dataloader))
            if (i+1) % 5 == 0:
                train_G_flag = True
            ...
            # Noise
            noise = torch.Tensor(b_size, 2, 5000).uniform_(-1, 1)
        ...
        msg = "Epochs:{}\t\tD_cost:{}\t\t D_wass:{}\t\tG_cost:{}".format(
