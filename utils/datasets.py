...
def load_ECG(ROOT):
    ...
    print(len(data_paf[0]), len(data_paf[0]))
    data_paf_ = []
    ...
    for item in data_paf:
      ...
      #########rec1=np.resize(rec1,(100,100))###########################################################3
      rec1 = cv2.resize(rec1, (100,100))
      #print(rec1.shape)
      #plt.imshow(rec1)
      #plt.show()

      # print('rec',len(rec1),len(rec1[0]))
    ...
    data_normal_ = []
    print(len(data_normal[0]), len(data_normal[0][0]))

    print('type',type(data_normal),type(data_normal[0]),type(data_normal[0][0]))
    print('len data_normal',len(data_normal),len(data_normal[0]),len(data_normal[0][0]))
    print('len data_paf',len(data_paf),len(data_paf[0]),len(data_paf[0][0]))
    print('len data_paf',len(data_paf_gan),len(data_paf_gan[0]),len(data_paf_gan[0][0]))
...
def get_ECG_data(seed, GAN_flag):
    ...
    # save_dataset = {"train":trian_data , "valid": val_data,"test":test_data}
    # torch.save(save_dataset , "save_dataset.pt")
